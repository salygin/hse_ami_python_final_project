...
|                                           | Настройка CI                                  | 3         |
| 3. Реализация ядра                        | Модуль загрузки презентации (`Loader`)        | 2         |
|                                           | Каркас PresentationAnalyzer и Rule            | 3         |
| 4. Реализация проверок (MVP)              | Правило списков (`ListRule`)                  | 3         |
|                                           | Правило шрифтов (`FontRule`)                  | 2         |
|                                           | Правило нумерации слайдов (`SlideNumberRule`) | 3         |
|                                           | Правило плотности текста (`TextDensityRule`)  | 2         |
|                                           | Правило стиля заголовков (`TitleStyleRule`)   | 4         |
...
Итого по проекту (оценочно): ~55–60 часов на команду.
...
Роли могут частично пересекаться: например, каждый участник пишет тесты к своим модулям и делает код-ревью друг другу.
